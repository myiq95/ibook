...
// Native TTS Bridge V9 - overrides web speechSynthesis with native AVSpeech
(function(){
  const isNative = !!(window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.tts);
  if(!isNative) return;
  window.NativeTTS = {
    speak(t,o={}){ window.webkit.messageHandlers.tts.postMessage({action:'speak', text:t, rate:o.rate||0.5, lang:o.lang||'ko-KR', voice:o.voiceId||null}) },
    pause(){ window.webkit.messageHandlers.tts.postMessage({action:'pause'}) },
    resume(){ window.webkit.messageHandlers.tts.postMessage({action:'resume'}) },
    stop(){ window.webkit.messageHandlers.tts.postMessage({action:'stop'}) },
    requestVoices(){ window.webkit.messageHandlers.ttsVoices.postMessage({}) }
  };
  const nativeSynth = {
    ...
    speak(u){ this._speaking=true; this._paused=false; this._currentUtter=u; window.NativeTTS.speak(u.text,{rate:u.rate||0.5, lang:u.lang||'ko-KR'}); setTimeout(()=>{try{u.onstart&&u.onstart();}catch(e){}},80); },
    ...
    pause(){ this._paused=true; window.NativeTTS.pause(); try{this._currentUtter.onpause&&this._currentUtter.onpause();}catch(e){} },
    resume(){ this._paused=false; window.NativeTTS.resume(); try{this._currentUtter.onresume&&this._currentUtter.onresume();}catch(e){} },
    getVoices(){return [];}
  };
  try{ Object.defineProperty(window,'speechSynthesis',{value:nativeSynth, configurable:true}); window.speechSynthesis=nativeSynth; }catch(e){ window.speechSynthesis=nativeSynth; }
  window.onNativeTTSState=function(state){
    ...
    else if(state==='finished' || state==='stopped'){ s._speaking=false; s._paused=false; if(u){try{u.onend&&u.onend();}catch(e){}} }
  };
  window.onVoicesLoaded=function(){};
  window.NativeTTS.requestVoices();
})();
