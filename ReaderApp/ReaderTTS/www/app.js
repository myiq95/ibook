...
// V14 Wrapper - fix after upload and keep current page
(function(){
  if(window.__v14Patched) return; window.__v14Patched=true;
  const origStart = window.startTts;
  window.startTts = function(){
    try{
      let q=[];
      try{ q=buildTtsQueue(); }catch(e){ q=[]; }
      if(!q||q.length===0){
        console.log("[V14] queue empty, retry 300ms");
        setTimeout(()=>{
          const q2=buildTtsQueue();
          if(!q2||q2.length===0){ if(typeof toast==='function') toast('읽을 텍스트가 없습니다'); if(typeof stopTTS==='function') stopTTS(); return; }
          State.tts.queue=q2;
          doStart(q2);
        }, 300);
        return;
      ...
      State.tts.queue=q;
      doStart(q);
    }catch(e){ console.error(e); if(origStart) origStart(); }
  };
  function doStart(queue){
    try{
      ...
      console.log("[V14] start from page", State.current.spread, "idx", startIdx);
      State.tts.idx=startIdx;
      State.tts.speaking=true; State.tts.paused=false;
